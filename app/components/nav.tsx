            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16">
                    ...
                    <div className="hidden md:flex space-x-8">
                        {routes.map((item) => (
                            <Link key={item.id} href={item.route} className="text-white/80 hover:text-white transition-colors duration-300">
                                {item.nav}
                            </Link>
                        ))}
                    </div>
            ...
            {menuOpen && (
                <div className="md:hidden bg-black/40 backdrop-blur-lg border-t border-white/10">
                    <div className="px-4 py-4 space-y-3">
                        {['Home', 'About', 'Our Projects'].map((item) => (
                            <a key={item} href="#" className="block text-white/80 hover:text-white transition-colors">
                                {item}
                            </a>
                        ))}
